...
if [ .$system_branch = .'master' ]; then
	...
	FUSION_MAJOR=$(git ls-remote --heads https://github.com/NocRoom/NocRoomPBX.git | cut -d/ -f 3 | grep -P '^\d+\.\d+' | sort | tail -n 1 | cut -d. -f1)
	FUSION_MINOR=$(git ls-remote --tags https://github.com/NocRoom/NocRoomPBX.git $FUSION_MAJOR.* | cut -d/ -f3 |  grep -P '^\d+\.\d+' | sort | tail -n 1 | cut -d. -f2)
