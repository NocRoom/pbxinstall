...
#count the users
admin_users=$(sudo -u postgres psql nocroompbx -Atc "select count(*) from v_users JOIN v_group_users USING (domain_uuid) where username='$system_username' and group_name = 'superadmin'")
...
if [ .$admin_users = .'0' ]; then
	...
	admin_uuids=$(sudo -u postgres psql nocroompbx -Atc "select v_users.user_uuid from v_users JOIN v_group_users USING (domain_uuid) where username='$system_username' and group_name = 'superadmin'")
	for admin_uuid in $admin_uuids; do
		...
		sudo -u postgres psql nocroompbx -c "update v_users SET password='$password_hash', salt='$user_salt' where user_uuid='$admin_uuid'"
